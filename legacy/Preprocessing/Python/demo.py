reference = open("D:\Research\Periodic Subgraph Mining\Preprocessing\Inputs\drosophila_subset_t1.txt").read().strip()
            ...
            if len(referenceCells) > 1:
                if len(lineCells) > 1:
                    ...
                    if referenceCells[1] == lineCells[2]:
                        out = open(
                            'D:\Research\Periodic Subgraph Mining\Preprocessing\Python\Timesteps with Edge Number\\t' + str(
                                i) + '.txt', 'a')
