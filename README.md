...
This section maps paper tables/figures to the scripts and outputs that generate them.

### Table-to-Output Mapping

| Paper Element | Description | Generated By | Output Location | Stage |
|---------------|-------------|--------------|-----------------|-------|
| **TABLE I** | Unique genes per period/support | ListMiner stats + analysis | `results/list_miner/*/results_stat.txt` | Stage 2 + Analysis |
| **TABLE II** | Subgraph properties (components, purity) | Postprocessing scripts 02-04 + utilities | `results/components/gene_numbers/` | Stage 3 + 5 |
| **TABLE III** | Cluster properties in largest components | ClusterONE + parsing | `results/clusters/individual/` | Stage 5 |
| **TABLE IV** | FlyEnrichr GO enrichment (p<0.05) | FlyEnrichr API + extraction | `results/enrichment/flyenrichr/` | Stage 6 |
| **TABLE V** | FuncAssociate enrichment (hub genes) | FuncAssociate web tool | `legacy/Funcassociate/` | External |
| **TABLE VI** | ClusterONE clustering statistics | ClusterONE output | `results/clusters/clusters.txt` | Stage 5 |
| **TABLE VII** | TF co-regulation enrichment | Custom analysis | *Script needed* | Analysis |
| **FIGURE 2** | Network visualization | Cytoscape (manual) | From `results/components/gene_names/` | Visualization |

### Required Analysis Scripts

Some paper tables require additional analysis scripts:

| Priority | Script Name | Purpose | Status |
|----------|-------------|---------|--------|
| HIGH | `generate_table1_gene_counts.py` | TABLE I statistics | ❌ To be created |
| HIGH | `generate_table2_properties.py` | TABLE II complete properties | ❌ To be created |
| MED | `generate_table3_clusters.py` | TABLE III cluster statistics | ❌ To be created |
| LOW | `tf_coregulation_analysis.py` | TABLE VII TF enrichment | ❌ To be created |

### Existing Scripts

✅ **Already implemented:**
- FlyEnrichr enrichment (TABLE IV): `scripts/05_utilities/extract_GO_terms.py`
- Component extraction (partial TABLE II): `scripts/03_postprocessing/02_remap_edges_to_graph.py`
- Purity calculation (TABLE II): `scripts/05_utilities/purity.py`
- Subgraph counting: `scripts/05_utilities/subgraph_count.py`
