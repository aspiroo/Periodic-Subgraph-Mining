...
public class ListMiner {
    ...
    public static int[][] pseArray = new int[21][31];
    public static int pseNum = 0;
    ...
    public void run() throws Exception {
        // Derive output directory from outputFilePath
        File outputFile = new File(outputFilePath);
        String outputDir = outputFile.getParent();
        this.graphOutputDir = outputDir;
        if (outputDir == null) outputDir = ".";

        // Clear existing p*s*.txt files in output dir
        File outDirFile = new File(outputDir);
        outDirFile.mkdirs();
        for (File f : outDirFile.listFiles()) {
            if (f.getName().matches("p\\ds.*\\.txt") || f.getName().matches("p\\d+s\\d+\\.txt")) {
                f.delete();
            }
        }
        ...
        // Create/clear the stats output file
        ...
        //flush all the remaining descriptors in the table
        ...
        psc = pseCount-pseNum;
        System.out.println("Number of PSEs:  " + psc);
        pseNum = pseCount;
    ...
    private void updateDescriptors(int time, Graph currentGraph) {
        for (int period = p; period <= Math.min(time-1, maxPeriod); period++) {
            ...
            for (int i = 0; i < dSize; i++) {
                ...
                if (result.scenario == Graph.NO_INTERSECTION) {
                    ...
                    for (int j = i; j < dSize; j++) {
                        flush(descriptors.get(j), period, phase, this.graphOutputDir);
                    }
                    break;
                    ...
                    flush(descriptors.get(i), period, phase, this.graphOutputDir);
    ...
    /**
     * Flushes the descriptor — writes to p{period}s{support}.txt in outputDir
     */
    private void flush(Descriptor descriptor, int period, int phase, String outputDir) {
        if (descriptor.getCount() >= s) {
            if (performSubsumation) {
            ...
            if (printGraphs && outputDir != null) {
                ...
                // Write to p{period}s{support}.txt
                String filename = outputDir + "/p" + period + "s" + descriptor.getCount() + ".txt";
                try {
