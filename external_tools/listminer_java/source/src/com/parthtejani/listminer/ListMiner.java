...
public class ListMiner {
    ...
    public static void main(String[] args) throws Exception {
      //for (int x = 2; x <= Math.floor(66/3); x++)
      //{
        //for (int y = 3; y <= Math.floor(66/x); y++)
        //{
          new ListMinerFactory(args).build().run();
          //System.out.println(psc);
          //pseArray[x-2][y-3] = psc;
          //s = s + 1;
        //}
        //p = p + 1;
      //}
      ...
      /*for(int i = 0; i < 20; i++)
      {
         //System.out.print("s" + i);
          ...
          for(int j = 0; j <= pseArray[i].length; j++)
          {
        	   System.out.print("p" + (i+2) +"s" + (j+3) + "->" );
             System.out.print(pseArray[i][j] + " ");
    ...
    private void flush(Descriptor descriptor, int period, int phase) {
        if (descriptor.getCount() >= s) {
            ...
            if (printGraphs) {
				...
				try {
						File file = new File("D:/Studies & Researches/Researches/Periodic Subgraph Mining/subgraph_count/individual/p2s"+s+".txt");
